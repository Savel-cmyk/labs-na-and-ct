...
class WineDataDriftMonitor:
    ...
    def generate_current_data(self, num_samples=200, drift_intensity=0.0):
        ...
        for i in range(num_samples):
            ...
            data.append({
                'description': description,
                'type': wine_type if random.random() > 0.1 else random.choice(wine_types),  # 10% шума в метках
                'timestamp': timestamp,
                'text_length': text_length,
                'word_count': word_count,
                'avg_word_length': text_length / word_count if word_count > 0 else 0,
...
def main():
    ...
    Теперь тестирует разные сценарии.
    """
    ...
    try:
        ...
        print("\nЗапуск серии тестов с разной интенсивностью дрейфа...")
        scenarios = [
            ("Без дрейфа", 0.0),
            ("Слабый дрейф (20%)", 0.2),
            ("Умеренный дрейф (50%)", 0.5),
            ("Сильный дрейф (80%)", 0.8)
        ]
        
        for scenario_name, drift_intensity in scenarios:
            print(f"\n{'='*50}")
            print(f"СЦЕНАРИЙ: {scenario_name}")
            print(f"{'='*50}")
            
            current_data = monitor.generate_current_data(
                num_samples=150, 
                drift_intensity=drift_intensity
            )
            
            analysis_result = monitor.run_drift_analysis(
                current_data,
                run_name=f"scenario_{scenario_name.replace(' ', '_')}"
            )
            
            status = "Пройден" if analysis_result['all_tests_passed'] else "⚠️ Дрейф обнаружен"
            print(f"\nРезультат: {status}")
